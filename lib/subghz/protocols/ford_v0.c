...
void subghz_protocol_decoder_ford_v0_get_string(void* context, FuriString* output) {
    ...
    // Show active button based on d-pad selection
    uint8_t display_btn = subghz_protocol_ford_v0_get_btn_code();
    const char* button_name = "??";
    if(display_btn == 0x01)
        button_name = "Lock";
    else if(display_btn == 0x02)
        button_name = "Unlock";
    else if(display_btn == 0x04)
        button_name = "Boot";
